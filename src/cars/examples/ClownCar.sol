...
contract ClownCar is Car {
    ...
    function takeYourTurn(Monaco.CarData[] calldata allCars, uint256 ourCarIndex) external override {
        ...
        bool shelled;
        bool lowCash = ourCar.balance < 3000 && ourCar.balance <= allCars[opps1Index].balance
            && ourCar.balance <= allCars[opps2Index].balance;
        ...
        if (50 < rng && !lowCash) {
            ...
            if (
                ourCarIndex != 0 && !shelled && allCars[ourCarIndex - 1].speed > ourCar.speed
                    && shellCost <= ourCar.balance
            ) {
